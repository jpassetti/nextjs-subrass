...
export function getAPStyleFormattedDate(dateString, momentLib) {
	const month = parseInt(momentLib(dateString).format("M"), 10);
	const monthDate = momentLib(dateString).format("D");
	const year = momentLib(dateString).format("YYYY");
	const dayOfTheWeek = momentLib(dateString).format("dddd");

	return `${dayOfTheWeek}, ${getAPStyleFormattedMonth(month)} ${monthDate}, ${year}`;
}

export function getAPStyleFormattedTime(dateString, momentLib) {
	const minutePortion = momentLib(dateString).format("mm");
	const hourPortion = momentLib(dateString).format("h");
	const minuteSuffix = minutePortion === "00" ? "" : `:${minutePortion}`;
	const formattedTimeAMPM = getFormattedAMPM(momentLib(dateString).format("a"));

	return `${hourPortion}${minuteSuffix} ${formattedTimeAMPM}`;
}
