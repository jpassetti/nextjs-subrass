...
export async function getPageBySlug(id) {
 ...
 try {
  const bySlugData = await fetchAPI(bySlugQuery, {
   variables: {
    slug: id,
   },
  });

  const bySlugPage = bySlugData?.pages?.edges?.[0]?.node;
  if (bySlugPage) {
   return bySlugPage;
  }
 } catch (error) {
  // Continue to URI fallback.
 }

 const uriAttempts = [id, `/${id}/`, `/${id}`];
 for (const uri of uriAttempts) {
  try {
   const byUriData = await fetchAPI(byUriQuery, {
    variables: {
     id: uri,
    },
   });
   ...
   if (byUriData?.page) {
    return byUriData.page;
   }
  } catch (error) {
   // Keep trying URI fallbacks.
  }
 }

 console.error("getPageBySlug failed", { id });
 return null;
}
