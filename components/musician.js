...
import {
 isGraduateOfSyracuseUniversity,
 truncateGraduationYear,
 doesEducationIncludeSyracuseUniversity,
 doesWorkIncludeSyracuseUniversity,
 displaySyracuseUniversityJobTitles,
} from "../lib/utilities";

const Musician = ({ data, teaser = false }) => {
 const { slug, content, instruments, featuredImage, peopleTypes } = data;
 const { prefix, firstName, middleInitial, lastName, suffix, education, work } =
 ...
 const fullName = (teaser) => {
  return teaser ? (
   <Fragment>
    <Span fontWeight="normal" fontSize="smaller">
    ...
    <Span fontWeight="bold" textTransform="uppercase">
     ...
     {education?.map((item, index) => {
      return isGraduateOfSyracuseUniversity(item.university)
       ? `${truncateGraduationYear(item.graduationYear, item.degreeType)} `
       : "";
     })}
    </Span>
   </Fragment>
   ...
   <Fragment>
    <Span fontWeight="400" fontColor="white" fontSize="smaller">
     {prefix ? prefix : ""} {firstName}{" "}
     {middleInitial ? `${middleInitial}.` : ""}{" "}
    </Span>
    <br />
 ...
 return teaser ? (
    ...
    <Link href={`/about/musicians/${slug}`}>
     <a>
      <ProfileImage
       ...
       isSUGraduate={
        education ? doesEducationIncludeSyracuseUniversity(education) : false
       }
  ...
  <Fragment>
   ...
   <div className={styles.showcase}>
    <Heading level="4" marginBottom="4" color="white">
     <Link href="/ensembles/2024-25">
      <a>&laquo; Musicians</a>
     </Link>
    </Heading>
    <Row alignItems="flex-start">
     ...
     <Col xs="12" sm="12" md="8">
      <Heading level="1" marginBottom="2" color="white" lineHeight="normal">
       {fullName()}
      </Heading>
      ...
      {instruments && (
       <Fragment>
        <Heading
         ...
         marginTop="4"
        >
         Instrument{instruments.edges.length > 1 ? "s" : ""}
        </Heading>
        <Heading level="3" marginBottom="2" color="white" fontWeight="400">
